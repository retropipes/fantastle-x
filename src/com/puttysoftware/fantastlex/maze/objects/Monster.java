...
public class Monster extends AbstractMovingObject {
	...
	if (FantastleX.getApplication().getMode() != Application.STATUS_BATTLE) {
	    ...
	    FantastleX.getApplication().getMazeManager().getMaze().postBattle(this, dirX, dirY, true);
	}
    }
	...
	if (arrowType == ArrowTypeConstants.ARROW_TYPE_ICE) {
	    ...
	    final int pz = FantastleX.getApplication().getMazeManager().getMaze().getPlayerLocationZ();
	    FantastleX.getApplication().getGameManager().morph(new IcedMonster(this.getSavedObject()), locX, locY, pz,
	...
	FantastleX.getApplication().getMazeManager().getMaze().updateMonsterPosition(move, dirX, dirY, this);
	this.activateTimer(1);
    }
