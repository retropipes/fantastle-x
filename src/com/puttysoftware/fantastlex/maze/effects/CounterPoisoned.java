...
public class CounterPoisoned extends MazeEffect {
	...
	// Apply the effect
	FantastleX.getApplication().getMazeManager().getMaze().doCounterpoisonAmulet();
    }

    @Override
    public void customTerminateLogic() {
	// Remove item that granted effect from inventory
	FantastleX.getApplication().getGameManager().getObjectInventory().removeItem(new CounterpoisonAmulet());
	// Undo the effect
	FantastleX.getApplication().getMazeManager().getMaze().undoPoisonAmulets();
    }
}