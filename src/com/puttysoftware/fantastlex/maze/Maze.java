...
public class Maze implements MazeConstants {
	...
	this.basePath = System.getProperty("java.io.tmpdir") + File.separator + "FantastleX" + File.separator + randomID
		+ ".maze";
	final File base = new File(this.basePath);
	final boolean success = base.mkdirs();
	if (!success) {
	    CommonDialogs.showErrorDialog("Maze temporary folder creation failed!", "FantastleX");
	}
    }

    // Static methods
    public static String getMazeTempFolder() {
	return System.getProperty("java.io.tmpdir") + File.separator + "FantastleX";
    }
	...
	temp.addLevel(FantastleX.getBattleMazeSize(), FantastleX.getBattleMazeSize(), 1);
