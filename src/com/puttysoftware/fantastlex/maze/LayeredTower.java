...
class LayeredTower implements Cloneable {
	...
	copy.savedTowerState = (LowLevelAMODataStore) this.savedTowerState.clone();
	...
	final int zLoc = this.getPlayerFloor();
	if (player) {
	    FantastleX.getApplication().getGameManager().setSavedMazeObject(saved);
	} else {
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
			...
			FantastleX.getApplication().getBattle().doBattleByProxy();
			// Respawn it
			this.generateOneMonster();
		    }
		} catch (final ArrayIndexOutOfBoundsException aioob) {
	...
	for (x = 0; x < lt.getColumns(); x++) {
		...
		for (z = 0; z < lt.getFloors(); z++) {
		    for (e = 0; e < MazeConstants.LAYER_COUNT; e++) {
			lt.setCell(FantastleX.getApplication().getObjects().readMazeObject(reader,
	...
	for (x = 0; x < sizeY; x++) {
		...
		for (z = 0; z < sizeZ; z++) {
			...
			this.savedTowerState.setCell(
				FantastleX.getApplication().getObjects().readMazeObject(reader, formatVersion), y, x, z,
				e);
		    }
		}
	    }
