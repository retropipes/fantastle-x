...
public abstract class AbstractContainer extends AbstractLock {
	...
	if (!app.getGameManager().isEffectActive(MazeEffectConstants.EFFECT_GHOSTLY)
	    ...
	    FantastleX.getApplication().getGameManager().addToScore(AbstractLock.SCORE_UNLOCK);
	} else {
	...
	if (!this.inside.getName().equals("Empty")) {
	    FantastleX.getApplication().showMessage(this.getName() + ": Contains " + this.inside.getName());
	} else {
	    FantastleX.getApplication().showMessage(this.getName() + ": Contains Nothing");
	}
    }
