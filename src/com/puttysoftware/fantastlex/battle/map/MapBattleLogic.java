...
public class MapBattleLogic extends AbstractBattle {
	...
	FantastleX.getApplication().getGameManager().addToScore(m.getExperience() + m.getGold());
	// Level Up Check
	...
	if (!this.resultDoneAlready) {
		...
		FantastleX.getApplication().getGameManager().addToScore(Math.max(1,
	...
	if (res != -1) {
	    ...
	    // AI Check
	    if (this.bd.getActiveCharacter().getTemplate().hasAI() && !FantastleX.inDebugMode()) {
		// Run AI
		this.waitForAI();
	...
	for (int x = 0; x < this.bd.getBattlers().length; x++) {
		...
		if (this.bd.getBattlerAIContexts()[x] != null
			...
			&& !FantastleX.inDebugMode() && this.bd.getBattlers()[x].isActive()
			&& this.bd.getBattlers()[x].getTemplate().isAlive()) {
