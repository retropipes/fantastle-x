...
class ObjectInventoryManager {
	...
	final String result = CommonDialogs.showInputDialog("Use which object?", "FantastleX", userChoices,
	...
	try {
		...
		if (result.equals(userChoices[x])) {
			...
			FantastleX.getApplication().showMessage("That item has no more uses left.");
			this.setUsingAnItem(false);
		    } else {
			FantastleX.getApplication().showMessage("Click to set target");
	...
	final String result = CommonDialogs.showInputDialog("Switch to which bow?", "FantastleX", userChoices,
	...
	try {
		...
		if (result.equals(userChoices[x])) {
			...
			FantastleX.getApplication().showMessage("That bow is out of arrows!");
		    } else {
			FantastleX.getApplication().showMessage(this.activeBow.getName() + " activated.");
		    }
		    return;
		}
	    }
	...
	if (this.usingAnItem() && app.getMode() == Application.STATUS_GAME) {
		...
		if ((target.isSolid() || !visible) && name.equals(new TeleportWand().getName())) {
		    this.setUsingAnItem(false);
		    FantastleX.getApplication().showMessage("Can't teleport there");
		}
		if (target.getName().equals(new Player().getName())) {
		    this.setUsingAnItem(false);
		    FantastleX.getApplication().showMessage("Don't aim at yourself!");
		}
		if (!target.isDestroyable() && name.equals(new AnnihilationWand().getName())) {
		    this.setUsingAnItem(false);
		    FantastleX.getApplication().showMessage("Can't destroy that");
		}
		if (!target.isDestroyable() && name.equals(new WallMakingWand().getName())) {
		    this.setUsingAnItem(false);
		    FantastleX.getApplication().showMessage("Can't create a wall there");
		}
		if (!target.isDestroyable() && name.equals(new FinishMakingWand().getName())) {
		    this.setUsingAnItem(false);
		    FantastleX.getApplication().showMessage("Can't create a finish there");
		}
		...
		if (!target.getName().equals(new Empty().getName()) && !target.getName().equals(new DarkGem().getName())
		    ...
		    this.setUsingAnItem(false);
		    FantastleX.getApplication().showMessage("Aim at either an empty space or a Dark Gem");
		}
		if (!target.getName().equals(new Empty().getName())
			&& !target.getName().equals(new LightGem().getName())
			&& name.equals(new DarkWand().getName())) {
		    this.setUsingAnItem(false);
		    FantastleX.getApplication().showMessage("Aim at either an empty space or a Light Gem");
		}
