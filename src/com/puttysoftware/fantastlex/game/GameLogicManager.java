...
public class GameLogicManager {
	...
	final AbstractMazeObject acted = FantastleX.getApplication().getMazeManager().getMazeObject(x, y, z,
	...
	try {
	    ...
	    m.restorePlayerLocation();
	    this.vwMgr.restoreViewingWindow();
	    FantastleX.getApplication().showMessage("Can't go outside the maze");
	} catch (final NullPointerException np) {
	    m.restorePlayerLocation();
	    this.vwMgr.restoreViewingWindow();
	    FantastleX.getApplication().showMessage("Can't go outside the maze");
	...
	if (this.actingRemotely) {
	    FantastleX.getApplication().getMazeManager().getMaze().setCell(decay, this.remoteCoords[0],
	...
	if (this.actingRemotely) {
	    FantastleX.getApplication().getMazeManager().getMaze().setCell(this.delayedDecayObject,
		    this.remoteCoords[0], this.remoteCoords[1], this.remoteCoords[2], MazeConstants.LAYER_OBJECT);
	} else {
	...
	try {
	    ...
	    FantastleX.getApplication().showMessage(gameName2 + " on " + gameName1);
