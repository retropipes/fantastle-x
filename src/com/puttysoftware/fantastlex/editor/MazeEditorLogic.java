...
public class MazeEditorLogic {
	...
	final MazeObjectList objectList = FantastleX.getApplication().getObjects();
	...
	try {
		...
		if (mo2 == null) {
		    FantastleX.getApplication().showMessage("This object has no properties");
		} else {
		    this.checkTwoWayTeleportPair(this.getLocationManager().getEditorLocationZ());
	...
	case TELEPORT_TYPE_N_WAY:
	    ...
	    FantastleX.getApplication().showMessage("Click to set teleport destination #" + (this.nWayDestID + 1));
	    break;
	...
	case TELEPORT_TYPE_INVISIBLE_BLOCK:
	    FantastleX.getApplication().showMessage("Click to set teleport destination");
	    break;
	...
	FantastleX.getApplication().showMessage("Click to set metal button target");
	final Application app = FantastleX.getApplication();
	...
	if (this.confirmNonUndoable("overwrite the active level with random data")) {
		...
		FantastleX.getApplication().getMazeManager().getMaze().fillLevelRandomlyCustom();
	    } else {
		FantastleX.getApplication().getMazeManager().getMaze().fillLevelRandomly();
	    }
	...
	if (this.confirmNonUndoable(
	    ...
	    FantastleX.getApplication().getMazeManager().getMaze().fillFloorAndLayer(
		    this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	...
	if (this.confirmNonUndoable("overwrite the active layer on the active level with random data")) {
	    ...
	    FantastleX.getApplication().showMessage("Level and layer randomly filled.");
	    FantastleX.getApplication().getMazeManager().setDirty(true);
	    this.redrawEditor();
	}
    }
	...
	if (this.confirmNonUndoable(
	    ...
	    if (FantastleX.getApplication().getMenuManager().useFillRuleSets()) {
		FantastleX.getApplication().getMazeManager().getMaze().fillFloorAndLayerRandomlyCustom(
			this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	    } else {
		FantastleX.getApplication().getMazeManager().getMaze().fillFloorAndLayerRandomly(
			this.getLocationManager().getEditorLocationZ(), this.getLocationManager().getEditorLocationE());
	    }
	    FantastleX.getApplication().showMessage("Floor and layer randomly filled.");
	    FantastleX.getApplication().getMazeManager().setDirty(true);
	    this.redrawEditor();
	}
    }
	...
	if (!this.goToDestMode) {
	    ...
	    FantastleX.getApplication().showMessage("Click a teleport to go to its destination");
	}
    }
	...
	if (this.goToDestMode) {
	    ...
	    final AbstractMazeObject there = FantastleX.getApplication().getMazeManager().getMazeObject(locX, locY,
		    locZ, MazeConstants.LAYER_OBJECT);
		...
		FantastleX.getApplication().showMessage("This object does not have a destination.");
	    }
	}
    }
