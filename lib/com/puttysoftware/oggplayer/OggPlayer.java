...
public class OggPlayer extends Thread {
	...
	while (!this.stop) {
	    try (AudioInputStream stream = AudioSystem.getAudioInputStream(this.url)) {
		// Get AudioInputStream from given file.
		if (stream != null) {
		    format = stream.getFormat();
		    decodedFormat = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, format.getSampleRate(), 16,
			    format.getChannels(), format.getChannels() * 2, format.getSampleRate(), false);
		    // Get AudioInputStream that will be decoded by underlying
		    // VorbisSPI
		    try (AudioInputStream decodedStream = AudioSystem.getAudioInputStream(decodedFormat, stream)) {
			try (SourceDataLine line = OggPlayer.getLine(decodedFormat)) {
			    if (line != null) {
				try {
